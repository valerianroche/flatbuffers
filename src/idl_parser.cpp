...
BOOST_STATIC_ASSERT_MSG(BASE_TYPE_UNION ==
...
bool LessFieldDefById(const FieldDef *a, const FieldDef *b) {
...
void Parser::ParseMetaData(Definition &def) {
  if (IsNext('(')) {
    for (;;) {
      const std::string & name = attribute_;
      Expect(kTokenIdentifier);
      if (known_attributes_.find(name) == known_attributes_.end())
      {
          std::string aContent = ". Known attributes are: ";
          for (std::set<std::string>::const_iterator itSet = known_attributes_.begin(); itSet != known_attributes_.end(); ++itSet) aContent += *itSet + " ";
        Error("user define attributes must be declared before use: " + name + aContent);
      }
        Value * e = new Value();
...
void Parser::ParseHash(Value &e, FieldDef* field) {
  ...
  switch (e.type.base_type) {
    case BASE_TYPE_INT:
    case BASE_TYPE_UINT: {
      const NamedHashFunction<uint32_t>::HashFunction & hash = FindHashFunction32(hash_name->constant.c_str());
    ...
    case BASE_TYPE_LONG:
    case BASE_TYPE_ULONG: {
      const NamedHashFunction<uint64_t>::HashFunction &  hash = FindHashFunction64(hash_name->constant.c_str());
...
void Parser::ParseEnum(bool is_union) {
  ...
  do {
    ...
    std::size_t prevsize = enum_def.vals.vec.size();
    std::size_t value = enum_def.vals.vec.size()
...
void Parser::ParseDecl() {
  ...
  if (fixed && force_align) {
    std::size_t align = static_cast<size_t>(atoi(force_align->constant.c_str()));
  ...
  if (!struct_def.fixed && fields.size()) {
    ...
    if (num_id_fields) {
      ...
      // been specified.
      std::sort(fields.begin(), fields.end(), &LessFieldDefById);
...
void Parser::MarkGenerated() {
  ...
  // been generated.
  for (std::vector<EnumDef *>::iterator it = enums_.vec.begin();
           it != enums_.vec.end(); ++it) {
    (*it)->generated = true;
  }
  for (std::vector<StructDef *>::iterator it = structs_.vec.begin();
...
Type Parser::ParseTypeFromProtoType() {
  ...
  Type type;
  for (type_lookup * tl = &lookup[0]; tl->proto_type; tl++) {
  ...
  try {
    ...
    while (token_ != kTokenEof) {
      if (proto_mode_) {
        ...
        Next();
        const std::string & root_type = attribute_;
        Expect(kTokenIdentifier);
...
template <typename T>
bool LessByName(const T *a, const T *b) {
...
template<typename T> void AssignIndices(const std::vector<T *> &defvec) {
  ...
  std::sort(vec.begin(), vec.end(), &LessByName<T>);
...
void Parser::Serialize() {
  ...
  flatbuffers::Offset<reflection::Schema> schema_offset = reflection::CreateSchema(
                         builder_,
