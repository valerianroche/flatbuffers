...
static void GenEnum(const Parser &parser, EnumDef &enum_def,
  ...
  if (enum_def.is_union) {
    ...
    // on the wrong type.
    const std::string & signature = "inline bool Verify" + enum_def.name +
...
static void GenTable(const Parser &parser, StructDef &struct_def,
  ...
  for (std::vector<FieldDef *>::const_iterator it = struct_def.fields.vec.begin();
    ...
    if (!field.deprecated) {  // Deprecated fields won't be accessible.
      ...
      const std::string & accessor = is_scalar
        ? "GetField<"
        : (IsStruct(field.value.type) ? "GetStruct<" : "GetPointer<");
      const std::string & offsetstr = NumToString(field.value.offset);
      std::string call =
          accessor +
      ...
      if (opts.mutable_buffer) {
        if (is_scalar) {
          ...
          const std::string & type = GenTypeGet(parser, field.value.type, " ", "", " *", true);
  ...
  code += "> Finish() {\n    flatbuffers::Offset<" + struct_def.name + "> o = flatbuffers::Offset<" + struct_def.name;
...
static void GenPadding(const FieldDef &field,
                       const boost::function<void (int bits)> &f) {
  if (field.padding) {
    for (int i = 0; i < 4; i++)
...
static void GenStructPadding(std::string &code, int &padding_id, int bits) {
    code += "  int" + NumToString(bits) +
            "_t __padding" + NumToString(padding_id++) + ";\n";
}

static void GenStructPadding2(std::string &code, int &padding_id, int bits) {
  (void)bits;
  code += ", __padding" + NumToString(padding_id++) + "(0)";
}

static void GenStructPadding3(std::string &code, int &padding_id, int bits) {
  (void)bits;
...
static void GenStruct(const Parser &parser, StructDef &struct_def,
  ...
  for (std::vector<FieldDef  *>::const_iterator it = struct_def.fields.vec.begin();
    ...
    code += field.name + "_;\n";
    GenPadding(field, boost::bind(&GenStructPadding, boost::ref(code), boost::ref(padding_id), _1));
  }
  ...
  for (std::vector<FieldDef  *>::const_iterator it = struct_def.fields.vec.begin();
    ...
    GenPadding(field, boost::bind(&GenStructPadding2, boost::ref(code), boost::ref(padding_id), _1));
  }
  code += " {";
  padding_id = 0;
  for (std::vector<FieldDef  *>::const_iterator it = struct_def.fields.vec.begin();
       ...
       ++it) {
    FieldDef &field = **it;
    GenPadding(field, boost::bind(&GenStructPadding3, boost::ref(code), boost::ref(padding_id), _1));
  }
  code += " }\n\n";
...
std::string GenerateCPP(const Parser &parser,
  ...
  for (std::vector<StructDef *>::const_iterator it = parser.structs_.vec.begin();
    ...
    const std::string & decl = "struct " + struct_def.name + ";\n";
  ...
  if (enum_code.length() || decl_code.length()) {
    ...
    include_guard_ident.erase(
      std::remove_if(include_guard_ident.begin(),
                     ...
                     std::not1(std::ptr_fun(&isalnum))),include_guard_ident.end());
    ...
    if (parser.root_struct_def_) {
      const std::string &name = parser.root_struct_def_->name;
      const std::string &qualified_name = parser.GetFullyQualifiedName(name);
      const std::string &cpp_qualified_name = TranslateNameSpace(qualified_name);
...
bool GenerateCPP(const Parser &parser,
                 ...
                 const std::string &file_name,
                 const GeneratorOptions &opts) {
    const std::string & code = GenerateCPP(parser, file_name, opts);
    return !code.length() ||
